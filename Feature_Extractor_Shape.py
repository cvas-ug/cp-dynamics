...
def main(train_loader,vali_loader,test_loader):
    ...
    with torch.no_grad():
        ...
        print ('acc (test):',acc)
    torch.save(alexnet.state_dict(),model_dir+'./alexnet_shape_dict.pth')
    return train,acc_train,vali,acc_vali,epochs,confusion_matrix

transforms=T.Compose([T.Resize((256,256)),T.ToTensor()])
root='./data'

cloth_train=PictureDataset(file_path='./Database/Real/depth/',csv_path='./csv_clothes/real/depth/LOOD_75_full.csv',idx_column=4,transforms=transforms)
cloth_test=PictureDataset(file_path='./Database/Real/depth/',csv_path='./csv_clothes/real/depth/LOOD_25_full.csv',idx_column=4,transforms=transforms)
