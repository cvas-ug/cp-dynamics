...
def move_average(dataloader,features,classifier,model):
    ...
    for t, input_ in enumerate(dataloader):
        ...
        movavg_light=mean(move_light)
        movavg_medium=mean(move_medium)
        movavg_heavy=mean(move_heavy)
        possibilities=[movavg_light,movavg_medium,movavg_heavy]
...
def statistics_method(pairs):
    ...
    th=[th_z*100,th_o*100,th_tw*100]
    f=[f_z*100,f_o*100,f_tw*100]
    ...
    plt.savefig(fig_dir+'MA_Weight.png')

transform=T.Compose([
    T.Resize((256,256)),
    T.ToTensor()
])
gartment_dataset=PictureDataset(file_path='./Database/Real/depth/',csv_path='./csv_clothes/real/depth/LOOD_25_full.csv',idx_column=6,transforms=transform)
